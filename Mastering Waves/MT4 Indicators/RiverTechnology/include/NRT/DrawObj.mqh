...
void DrawObj_Process(){   
   ...
   if (level_price == level_bias) {      
      if (level_price == level_time){
         SetText("text_a1_" + GetTimeFrame(), offset+t+" "+b+" "+p+" " + DoubleToStr(level_price, Digits), 0, level_price, MediumOrchid);          
      }else{
         SetText("text_a1_" + GetTimeFrame(), offset+b+" "+p+" " +DoubleToStr(level_price, Digits), 0, level_price, MediumOrchid);       
         SetText("text_a2_" + GetTimeFrame(), offset+t+" " + DoubleToStr(level_time, Digits), 0, level_time, Brown);          
      }
   }else{            
      if (level_bias == level_time) {
         SetText("text_a1_" + GetTimeFrame(), offset+t+" "+b+" " + DoubleToStr(level_bias, Digits), 0, level_bias, DodgerBlue);                
         SetText("text_a2_" + GetTimeFrame(), offset+p+" " + DoubleToStr(level_price, Digits), 0, level_price, MediumOrchid);                
      }else{
         if (level_price == level_time){
            SetText("text_a1_" + GetTimeFrame(), offset+t+" "+p+" " + DoubleToStr(level_bias, Digits), 0, level_time, Brown);                
            SetText("text_a2_" + GetTimeFrame(), offset+b+" " + DoubleToStr(level_bias, Digits), 0, level_bias, DodgerBlue);                
         }else{         
            SetText("text_a1_" + GetTimeFrame(), offset+b+" " + DoubleToStr(level_bias, Digits), 0, level_bias, DodgerBlue);                
            SetText("text_a2_" + GetTimeFrame(), offset+p+" " + DoubleToStr(level_price, Digits), 0, level_price, MediumOrchid);                
            SetText("text_a3_" + GetTimeFrame(), offset+t+" " + DoubleToStr(level_time, Digits), 0, level_time, Brown);                      
         }
      }
   }      
...
void Draw_MotionLine(){
   ...
   for (int i = 1; i <= MLLastBarIndex(); i++){    
      ...
      DrawLine("line_ml_"+i+"_"+MathRand(), "", 
...
void Draw_Peaks(){
   ...
   for (int i = 1; i <= MLLastPeakIndex(); i++){          
      ...
      DrawLine("line_peak_"+i+"_"+MathRand(), "", 
               GetShift(time, Period()), GetShift(time1, Period()),
